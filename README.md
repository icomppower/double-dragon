...
- `node playtest.mjs http://127.0.0.1:8124 [--mobile]` — headless Chrome (needs `npm install`): kit and all eight rigs load, real menu clicks, intro, keyboard walk/depth/punch, limb animation, autoplay for 20 s, HUD tracking, continue flow, hideout with the captive, the elite boss tint, pause/quit, the full ending, and on 390×844 the joystick, three buttons and the pause button hit-tested with elementFromPoint plus a real touch tap and drag. 37 desktop / 47 mobile checks.
