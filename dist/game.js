...
function frame(now) {
  ...
  const dt = Math.min(1.0, (now - lastT) / 1000 || 0); lastT = now;   // a slow renderer (software GL) still gets real time: the sim is cheap, so up to 60 steps a frame
  ...
  if (mode === 'ending') { endT += dt; if (sim) { acc += dt; let k = 0; while (acc >= DT && k < 60) { step(sim, {}, DT); acc -= DT; k++; } syncWorld(sim, dt); updateCamera(sim, dt); renderer.render(scene, camera); } if (endT > 14 || (endT > 2 && (keys.enter || keys.punch || touch.punch))) toMenu(); return; }
